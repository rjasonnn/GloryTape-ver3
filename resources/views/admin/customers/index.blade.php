            ...
            <div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
                ...
                <div class="mt-3 sm:mt-0 sm:ml-4">
                    <form action="{{ route('customers.create') }}" method="POST">
                        @csrf
                        @method('GET')
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Create New Customer</button>
                    </form>
                            ...
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Address</th>
                                        <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Phone</th>
                                    ...
                                    @foreach($customers as $customer)
                                        <tr>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 sm:pl-6">{{ $customer->name }}</td>
                                            ...
                                            <td class=" whitespace-nowrap py-4 pl-3 ">
                                                <form action="{{ route('customers.show', $customer->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('GET')
                                            ...
                                            <td class=" whitespace-nowrap py-4 pl-3 ">
                                                <form action="{{ route('customers.edit', $customer->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('GET')
