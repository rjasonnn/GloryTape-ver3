    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Colour</h3>
                <div class="mt-3 sm:mt-0 sm:ml-4">
                    <form action="{{ route('warnas.create') }}" method="POST">
                        @csrf
                        @method('GET')
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Create New Colour</button>
                    </form>
                            ...
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Code</th>
                                    ...
                                    @foreach($warnas as $warna)
                                        <tr>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 sm:pl-6">{{ $warna->name }}</td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $warna->code }}</td>
                                            <td class=" whitespace-nowrap py-4 pl-3 ">
                                                <form action="{{ route('warnas.show', $warna->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('GET')
                                            ...
                                            <td class=" whitespace-nowrap py-4 pl-3 ">
                                                <form action="{{ route('warnas.edit', $warna->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('GET')
                                            ...
                                            <td class=" whitespace-nowrap py-4 pl-3 ">
                                                <form action="{{ route('warnas.destroy', $warna->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
